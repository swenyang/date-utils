...
describe('Test dateUtils.age()', () => {
    ...
    it('test age calculation (addAgeAfterBirthday)', () => {
        dateUtils.seed('2016-08-04')
        expect(dateUtils.age('abc')).to.be.equal(0)
        expect(dateUtils.age('2016-08-05')).to.be.equal(0)
        expect(dateUtils.age('2016-08-04')).to.be.equal(0)
        expect(dateUtils.age('2016-08-03')).to.be.equal(0)
        const min = new Date('1916-08-03')
        const max = new Date('2016-08-02')
        let age = 0
        for (let i = max.getTime(); i >= min.getTime(); i -= 24 * 3600000) {
            ...
            expect(dateUtils.age(d)).to.be.equal(age)
        }
    })
})
